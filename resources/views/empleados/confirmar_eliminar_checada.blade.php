...
<div class="container-fluid">
	<div class="row">
		<form class="form-horizontal" role="form" method="POST" action="{{ route('eliminar_checada', $checada->idChecada}}" id="confirmar-form"  >
			...
			<h3>Confirmar</h3>
				<table class="table table-striped table-bordered table-hover dataTable no-footer" border="2" width="100%" rules="rows" style='text-transform:uppercase'>
					...
					<tr>
						<td>{{$checada->idEmpleado}}</td>
						<td>{{$checada->idCliente}}</td>
						<td>{{$checada->idTurno}}</td>
						<td>{{$checada->fecha}}</td>
				...
				<div class="form-group">
				    <center>
						<a type="submit" class="btn btn-primary" value="validate" style="margin-right: 15px;">
							Guardar 
						</a>
					</center>
				</div>
